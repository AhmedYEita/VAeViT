# Created on WEd Aug 9 13:50:00 2023 - Ahmed Yasser Eita
# from "VAE_ModelNet10_64_animecode_org"
...
path="D://Master//Datasets//3D//Multi-view images//ModelNet10//modelnet10_80o_12v"
class_names=os.listdir(path) #list all the class names and append them in a avriable

train_image_paths, train_labels = [], []
for i1 in class_names: #bathtub
    dir1 = os.path.join(path,i1+"//train") #dataset/bathtub/train
    for i2 in os.listdir(dir1):
        train_image_paths.append(os.path.join(dir1, i2))
...
for i1 in class_names: #bathtub
    ...
    for i2 in os.listdir(dir1):
        test_image_paths.append(os.path.join(dir1, i2))
        for n,j in enumerate(class_names):
            if j==i1:
...
## when using plt instead of cv2 in imread, it directly reads the image in a float format and divide by 255
...
# sample = train_dataset.unbatch().take(25)
...
# CLASSIFICATION BY LATENT VECTOR ---------------------------------------------
...
conf_plot=sns.heatmap(conf_table,annot=True,cmap='BuGn',fmt='g') #BuGn, Greys
plt.title('64p gray Animecode_org not shuffled %i l_d, %i e, classification acc: %.0f%%' %(latent_dim,epochs,(acc*100)))
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.savefig('output/conf_matrix_AnimeCode_org_notshuffled_pixels_{:03d}_LatentDim_{:03d}_epoch_{:03d}_2.pdf'.format(img_height, latent_dim, epoch), bbox_inches='tight')
plt.show()
# =============================================================================


# Save arrays -----------------------------------------------------------------
with open('models/z_train9600_384ld_100e_1.pkl', 'wb') as f:
    pkl.dump(z_train, f, protocol=pkl.HIGHEST_PROTOCOL)
    
with open('models/y_train9600_384ld_100e_1.pkl', 'wb') as f:
    pkl.dump(train_labels, f, protocol=pkl.HIGHEST_PROTOCOL)
#--------------------------------------------------------
with open('models/z_train9600_384ld_100e_1.pkl', 'rb') as f:
    z_train = pkl.load(f)
 
with open('models/y_train9600_384ld_100e_1.pkl', 'rb') as f:
    y_train = pkl.load(f)
# =============================================================================
